...
# C1 = ob.block_tables(sample_A, sample_B, 'NAME', 'NAME', word_level=False, q_val=3, overlap_size=5,
...
#                 strategy='mean')
result = em.select_matcher([dt, rf, svm, ln, lg], table=feature_vectors_train,
        exclude_attrs=['_id', 'ltable_ID', 'rtable_ID', 'label'],
        k=5,
        target_attr='label', metric_to_select_matcher='f1', random_state=0)
print(result['cv_stats'])

dt.fit(table=feature_vectors_train, exclude_attrs=['_id', 'ltable_ID', 'rtable_ID', 'label'], target_attr='label')
predictions = dt.predict(table=feature_vectors_test, exclude_attrs=['_id', 'ltable_ID', 'rtable_ID', 'label'],
